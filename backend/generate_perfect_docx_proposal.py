...
Unicode-Perfect Microsoft Word Proposal Generator (.docx)
Embeds OpenXML Complex Script (w:cs, w:lang="bn-BD") tags to ensure 100% flawless Bengali text shaping in Word & WPS.
"""
...
FONT_FAMILY = "Kalpurush"  # Standard Bengali Unicode Font installed on Windows

def apply_bengali_font_to_run(run, font_size=10.5, bold=False, italic=False, color=None):
    """
    Applies OpenXML complex script tags to a python-docx run so Microsoft Word
    shapes Bengali conjuncts, vowel marks (কার), and hasantas with 100% accuracy.
    """
    ...
    # 1. OpenXML rFonts mapping for ASCII, High-ANSI, East-Asia, and Complex Script (cs)
    rFonts_xml = (
    ...
    rPr.append(parse_xml(rFonts_xml))
    
    # 2. Language tag for Bengali (Bangladesh)
    lang_xml = f'<w:lang {nsdecls("w")} w:val="bn-BD" w:eastAsia="bn-BD" w:bidi="bn-BD"/>'
    rPr.append(parse_xml(lang_xml))
    
    # 3. Explicit Complex Script flag
    cs_xml = f'<w:cs {nsdecls("w")}/>'
    rPr.append(parse_xml(cs_xml))
...
def generate_perfect_docx():
    ...
    for style_name in ['Normal', 'Heading 1', 'Heading 2', 'Heading 3']:
        if style_name in styles:
            ...
            rPr.append(parse_xml(
                f'<w:rFonts {nsdecls("w")} w:ascii="{FONT_FAMILY}" w:hAnsi="{FONT_FAMILY}" w:cs="{FONT_FAMILY}" w:eastAsia="{FONT_FAMILY}"/>'
            ))

    # Header Bar
    p_hdr = doc.add_paragraph()
    p_hdr.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    r_hdr = p_hdr.add_run("জাতীয় বিশ্ববিদ্যালয়, বাংলাদেশ • প্রাতিষ্ঠানিক প্রজেক্ট প্রপোজাল ২০২৬")
    apply_bengali_font_to_run(r_hdr, font_size=9, color=COLOR_SLATE)

    # Main Title
    p_title = doc.add_paragraph()
    p_title.paragraph_format.space_before = Pt(4)
    p_title.paragraph_format.space_after = Pt(2)
    r_title = p_title.add_run("জাতীয় বিশ্ববিদ্যালয় স্মার্ট AI অ্যাসিস্ট্যান্ট ও সেন্ট্রালাইজড সাপোর্ট ইকোসিস্টেম")
    apply_bengali_font_to_run(r_title, font_size=18, bold=True, color=COLOR_EMERALD_DARK)

    p_sub = doc.add_paragraph()
    p_sub.paragraph_format.space_after = Pt(12)
    r_sub = p_sub.add_run("৩২ লক্ষ শিক্ষার্থী, ২ লক্ষ শিক্ষক ও ২,২৬০টি অধিভুক্ত কলেজের জন্য আধুনিক কৃত্রিম বুদ্ধিমত্তা ভিত্তিক সেবা অটোমেশন")
    apply_bengali_font_to_run(r_sub, font_size=10.5, color=COLOR_SLATE)

    # Statistics 4-Card Box Table
    ...
    for i, (title, sub, bg) in enumerate(stats):
        ...
        apply_bengali_font_to_run(r1, font_size=11.5, bold=True, color=COLOR_EMERALD_DARK)
        r2 = p.add_run(sub)
        apply_bengali_font_to_run(r2, font_size=8.5, color=COLOR_SLATE)
    ...
    r_h1 = h1.add_run("১. প্রজেক্টের পটভূমি ও প্রয়োজনীয়তা (Executive Background & Rationale)")
    apply_bengali_font_to_run(r_h1, font_size=13.5, bold=True, color=COLOR_EMERALD_DARK)
    ...
    apply_bengali_font_to_run(r1, font_size=10, color=COLOR_TEXT)
    ...
    apply_bengali_font_to_run(r2, font_size=10, color=COLOR_TEXT)

    # 2. Stakeholder Value Realization
    h2 = doc.add_heading(level=1)
    h2.paragraph_format.space_before = Pt(12)
    h2.paragraph_format.space_after = Pt(6)
    r_h2 = h2.add_run("২. স্টেকহোল্ডার ভিত্তিক সুবিধার বিশ্লেষণ (Stakeholder Value Realization)")
    apply_bengali_font_to_run(r_h2, font_size=13.5, bold=True, color=COLOR_EMERALD_DARK)

    stakeholders = [
        ("🎓 শিক্ষার্থীদের জন্য সুবিধা (৩.২ মিলিয়ন শিক্ষার্থী)", [
            "তাৎক্ষণিক ২৪/৭ উত্তর: বাংলা বা ইংরেজিতে ভর্তি যোগ্যতা, সংশোধিত রুটিন ও রেজাল্ট জানার সুযোগ (< ০.০০১ সেকেন্ড)।",
            "অফিসিয়াল সাপোর্ট টোকেন: ইএমএস লগইন, ফরম পূরণ বা সনদ জটিলতায় ট্র্যাকিং নম্বর (NU-2026-XXXXXX) সহ সমাধান।",
            "বিপুল অর্থ ও সময় সাশ্রয়: গাজীপুর মূল ক্যাম্পাসে আসার যাতায়াত ও থাকা-খাওয়ার খরচ সম্পূর্ণ দূর হবে (বার্ষিক ৫০ কোটি+ টাকা সাশ্রয়)।",
    ...
    for title, points, bg in stakeholders:
        ...
        apply_bengali_font_to_run(r_head, font_size=10.5, bold=True, color=COLOR_EMERALD_DARK)
        for pt in points:
            r_pt = p.add_run(f"• {pt}\n")
            apply_bengali_font_to_run(r_pt, font_size=9.5, color=COLOR_TEXT)
        doc.add_paragraph().paragraph_format.space_after = Pt(3)

    # 3. Core Capabilities & Tech Stack
    h3 = doc.add_heading(level=1)
    h3.paragraph_format.space_before = Pt(12)
    h3.paragraph_format.space_after = Pt(6)
    r_h3 = h3.add_run("৩. প্ল্যাটফর্মের প্রধান কার্যাবলী ও প্রযুক্তিগত উদ্ভাবন")
    apply_bengali_font_to_run(r_h3, font_size=13.5, bold=True, color=COLOR_EMERALD_DARK)
    ...
    for i, h in enumerate(headers):
        ...
        p = cell.paragraphs[0]
        r = p.add_run(h)
        apply_bengali_font_to_run(r, font_size=9, bold=True, color=RGBColor(255, 255, 255))
    ...
    for row_data in tech_rows:
        ...
        for i, val in enumerate(row_data):
            ...
            p = cell.paragraphs[0]
            r = p.add_run(val)
            apply_bengali_font_to_run(r, font_size=8.5, bold=(i == 0), color=COLOR_TEXT)
    ...
    r_h4 = h4.add_run("৪. প্ল্যাটফর্মের ইন্টারফেস ও কার্যপ্রণালীর চিত্র (Interface Demonstration)")
    apply_bengali_font_to_run(r_h4, font_size=13.5, bold=True, color=COLOR_EMERALD_DARK)
    ...
    for img_name, caption in images_to_embed:
        ...
        if img_path.exists():
            ...
            apply_bengali_font_to_run(r_cap, font_size=8.5, bold=True, color=COLOR_SLATE)

    # 5. Support Services & SLA
    h5 = doc.add_heading(level=1)
    h5.paragraph_format.space_before = Pt(12)
    h5.paragraph_format.space_after = Pt(6)
    r_h5 = h5.add_run("৫. সাপোর্ট সার্ভিস ক্যাটালগ ও সমাধানের সময়সীমা (SLA)")
    apply_bengali_font_to_run(r_h5, font_size=13.5, bold=True, color=COLOR_EMERALD_DARK)
    ...
    # 6. Strategic Roadmap & ROI
    h6 = doc.add_heading(level=1)
    h6.paragraph_format.space_before = Pt(12)
    h6.paragraph_format.space_after = Pt(6)
    r_h6 = h6.add_run("৬. বাস্তবায়ন রোডম্যাপ ও প্রাতিষ্ঠানিক সুবিধা (ROI)")
    apply_bengali_font_to_run(r_h6, font_size=13.5, bold=True, color=COLOR_EMERALD_DARK)
    ...
    for i, (title, sub, bg) in enumerate(roi_cards):
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r1 = p.add_run(f"{title}\n")
        apply_bengali_font_to_run(r1, font_size=10, bold=True, color=COLOR_EMERALD_DARK)
        r2 = p.add_run(sub)
        apply_bengali_font_to_run(r2, font_size=8, color=COLOR_TEXT)
    ...
    r_h7 = h7.add_run("৭. সুপারিশ ও প্রাতিষ্ঠানিক অনুমোদনের আবেদন (Sign-Off & Recommendation)")
    apply_bengali_font_to_run(r_h7, font_size=13.5, bold=True, color=COLOR_EMERALD_DARK)
    ...
    apply_bengali_font_to_run(r_rec, font_size=10, color=COLOR_TEXT)
    ...
    for i, sig_text in enumerate(sigs):
        ...
        apply_bengali_font_to_run(r_txt, font_size=9, bold=True, color=COLOR_NAVY)

    docx_path = PROPOSAL_DIR / "project_proposal.docx"
    doc.save(str(docx_path))
    print(f"[SUCCESS] Unicode-perfect DOCX saved: {docx_path} ({docx_path.stat().st_size} bytes)")
